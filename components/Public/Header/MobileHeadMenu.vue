  ...
  <div class="right-head">
    <div class="menu">
      <div v-for="(item, index) in menuList" :key="item.id" class="fa-path">
        <nuxt-link @click="item.path == '/' ? showChildMenu(index) : mbToLink(item)"
          :class="[item.isChildVisible ? `a-link-${item.id}` : '', 'a-link']">{{ item.title }}</nuxt-link>
        <transition name="fade">
          <div v-if="item.isChildVisible" class="sub-menu">
            ...
            > -->
            <div v-for="child in item.childrenList" :key="child.id" class="son-menu" @click="pathIsTrue" :class="[
              child.path === '/services/ai-screening'
                ? 'is_ai_screening'
                : '',
            ]">
              <nuxt-link :to="child.path == '/' ? '#' : child.path"><span :class="[
                child.path === '/services/ai-screening'
                  ? 'ai_screening_title'
                  : '',
              ]">{{ child.title }}</span>
              ...
              <div v-if="isShowChildList" class="three-level">
                <div v-for="threeLevel in child.threeLevelList" :key="threeLevel.id">
                  <nuxt-link :to="threeLevel.path"><span>{{ threeLevel.title }}</span></nuxt-link>
                </div>
              </div>
            </div>
          </div>
